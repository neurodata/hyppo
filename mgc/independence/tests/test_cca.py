...
class TestCCAErrorWarn:
    ...
    def test_error_notndarray(self):
        ...
        x = np.arange(20)
        y = [5] * 20
        assert_raises(ValueError, CannCorr().test, x, y)
        assert_raises(ValueError, CannCorr().test, y, x)
    ...
    def test_error_nans(self):
        ...
        x[0] = np.nan
        assert_raises(ValueError, CannCorr().test, x, x)
    ...
    def test_error_reps(self, reps):
        ...
        x = np.arange(20)
        assert_raises(ValueError, CannCorr().test, x, x, reps=reps)

    def test_warns_reps(self):
        # raises warning when reps is less than 1000
        x = np.arange(20)
        reps = 100
        assert_warns(RuntimeWarning, CannCorr().test, x, x, reps=reps)
