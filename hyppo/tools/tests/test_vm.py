...
class TestCleanInputsVM:
    ...
    def setup_method(self):
        ...
        self.Ts_strings = np.array([treatments[i % len(treatments)] for i in range(self.n_samples)])
        
        # Create pandas categorical treatment
        self.Ts_pandas_cat = pd.Series(
            pd.Categorical(['A', 'B', 'C', 'A', 'B'] * (self.n_samples // 5), 
                           categories=['A', 'B', 'C'])
        )
    ...
    def test_ts_categorical_conversion(self):
        ...
        assert_equal(cleaner.unique_treatments, np.array(['control', 'treatment_A', 'treatment_B']))
        assert cleaner.K == 3
    ...
    def test_propensity_formula_generation(self):
        ...
        cleaner = _CleanInputsVM(self.Ts_binary, self.Xs_df, prop_form_rhs=custom_formula)
    ...
    def test_design_matrix_construction(self):
        ...
        cleaner = _CleanInputsVM(self.Ts_binary, Xs_positive, prop_form_rhs=custom_formula)
    ...
    def test_custom_formula_with_numpy_array(self):
        ...
        with pytest.raises(ValueError, match="propensity formula"):
            cleaner = _CleanInputsVM(self.Ts_binary, self.Xs, prop_form_rhs=custom_formula)
...
class TestVectorMatch:
    ...
    def test_vector_matching_with_one_oddball_per_group(self):
        ...
        for i in range(nrep):
            ...
            included_samples = sum([i in retained_ids for i in range(1, 199)]) / 198 > 0.8
    ...
    def test_error_when_no_samples_retained(self):
        ...
        rng = np.random.RandomState(123456789)
        sim_low = cate_sim("Sigmoidal", n=200, p=3, balance=0,
                           alpha=10, beta=1, random_state=rng)
    ...
    def test_vm_increases_covariate_overlap(self):
        ...
        ov_before = approx_overlap(
            sim_mod['Xs'][sim_mod['Ts'] == 0], 
            sim_mod['Xs'][sim_mod['Ts'] == 1]
        )
        
        ov_after = approx_overlap(
            Xs_tilde[Ts_tilde == 0], 
            Xs_tilde[Ts_tilde == 1]
        )
    ...
    def test_overlap_improvement_varies_with_balance(self):
        ...
        sim_low_balance = cate_sim("Sigmoidal", n=200, p=1, balance=0.3, random_state=rng)
        sim_high_balance = cate_sim("Sigmoidal", n=200, p=1, balance=0.8, random_state=rng)
        ...
        low_after = approx_overlap(
            Xs_tilde_low[Ts_tilde_low == 0], 
            Xs_tilde_low[Ts_tilde_low == 1]
        )
        ...
        high_after = approx_overlap(
            Xs_tilde_high[Ts_tilde_high == 0], 
            Xs_tilde_high[Ts_tilde_high == 1]
        )
        ...
        assert low_improvement > 0, "Low balance data should show positive overlap improvement"
        assert high_improvement > 0, "High balance data should show positive overlap improvement"