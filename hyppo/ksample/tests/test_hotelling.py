...
class TestHotelling:
    @pytest.mark.parametrize(
        "n, obs_stat, obs_pvalue, auto",
        [(1000, 0.005062841807278008, 1.0, True), (100, 8.24e-5, 0.001, False)],
    )
    def test_twosamp_linear_oned(self, n, obs_stat, obs_pvalue, auto):
        np.random.seed(123456789)
        x, y = rot_2samp(linear, n, 1, noise=False)
        stat, pvalue = Hotelling().test(x, y, auto=auto)
...
class TestHotellingErrorWarn:
    ...
    def test_error_notndarray(self):
        ...
        y = [5] * 20
        z = np.arange(5)
        assert_raises(ValueError, Hotelling().test, x, y, z)
