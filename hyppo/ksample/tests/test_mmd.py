...
class TestMMD:
    ...
    def test_mmd_linear_oned(self, n, obs_stat, obs_pvalue):
...
class TestMMDErrorWarn:
    ...
    def test_error_notndarray(self):
        ...
        y = [5] * 20
        z = np.arange(5)
        assert_raises(ValueError, MMD().test, x, y, z)
    ...
    def test_error_lowsamples(self):
        ...
        x = np.arange(3)
        y = np.arange(3)
        assert_raises(ValueError, MMD().test, x, y)

    def test_error_nans(self):
        # raises error if inputs contain NaNs
        x = np.arange(20, dtype=float)
        x[0] = np.nan
        assert_raises(ValueError, MMD().test, x, x)
