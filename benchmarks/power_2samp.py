...
class _ParallelP(object):
    ...
    def __init__(self, test, ksim, sim, n, p, noise, rngs):
        self.test = test()
        self.ksim = ksim
        self.sim = sim
    ...
    def __call__(self, index):
        if (
            ...
            x, y = self.ksim(self.sim, self.n, self.p)
        else:
            x, y = self.ksim(self.sim, self.n, self.p, noise=self.noise)
...
def _perm_test(
    test, ksim, sim, n=100, p=1, noise=False, reps=100, workers=1, random_state=None
):
    r"""
    ...
    parallelp = _ParallelP(
        test=test, ksim=ksim, sim=sim, n=n, p=p, noise=noise, rngs=rngs
    )
